...
did_note_take_seriously = ['MEGp0106','MEGp0119','MEGp0122','MEGp0125','MEGp0143','MEGp0165']
did_not_understand = ['MEGp0118','MEGp0101','MEGp0100']
...
def median(vals):
        if(len(vals)>0): return stat.median(vals)
        else: return float("NaN")



def make_csv(subs, keys_to_store,filename='/MEGp_results.csv',include_block_level=True):
                        ...
                        for i in range(len(all_sets)):
                                ...
                                if(set_key not in s['analyzed']['sets']):
                                        ...
                                        for key in keys_to_store:
                                                ...
                                                if(include_block_level):
                                                        for block_num in range(max_blocks):
                                                                if(block_num not in s['analyzed']['sets'][set_key]['blocks']): values.append(0)
                                                                else: values.append(s['analyzed']['sets'][set_key]['blocks'][block_num][key])
...
for s in all_subjects: #every subject in the cohort
        if(s['id'].startswith("MEGp00")): continue #sets starting with MEGp00 belong to the 1st cohort where the neither buttons were removed
        ...
        for i,block in enumerate(s['blocks']): #every block for the subject
                ...
                for Q in block['choice']: #Q is every question/trial in the block
                        ...
                        elif (Q['response'] == 'neither'):
                                # print("pressed neither, moving to next trial")
                                continue
                        else:
        ...
        for set in s['analyzed']['sets']:
                ...
                if(necklace['trials_total']!=0):
                        ...
                        necklace['rt_shifted'] = median(necklace['rt_shifted'])
                        necklace['rt_swapped'] = median(necklace['rt_swapped'])
                        necklace['rt'] = median(necklace['rts'])
                ...
                for key in s['analyzed']['sets'][set]['blocks']:
                        ...
                        if (b_necklace['trials_total'] != 0):
                                ...
                                b_necklace['rt_shifted'] = median(b_necklace['rt_shifted'])
                                b_necklace['rt_swapped'] = median(b_necklace['rt_swapped'])
                                b_necklace['rt'] = median(b_necklace['rts'])
